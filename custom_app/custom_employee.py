...
def get_employees_with_birthday_in_current_month():
    ...
    employees = frappe.get_all("Employee",
        ...
        fields=["name", "employee_name", "date_of_birth","anniversary_date"],
    )
    curr_mon={}
    for emp in employees:
        if emp.date_of_birth and emp.date_of_birth.month == cur_month:
            if emp.date_of_birth in curr_mon:
                ...
                curr_mon[str(emp.date_of_birth)[5:]]=[(emp.employee_name,"birthday")]
        if emp.anniversary_date and emp.anniversary_date.month == cur_month:
            if emp.anniversary_date in curr_mon:
                curr_mon[str(emp.anniversary_date)[5:]]+=[(emp.employee_name,"anniversary")]
            else:
                curr_mon[str(emp.anniversary_date)[5:]]=[(emp.employee_name,"anniversary")]
